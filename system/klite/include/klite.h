...
/**
 * @brief 从当前线程fork一个新线程
 * @retval 如果当前线程是父线程返回新线程句柄, 否则返回NULL
 * @warning 如果fork失败, errno会被设置, 随后返回NULL
 */
kl_thread_t kl_thread_fork(void);
 ...
 * @retval 继续迭代返回true, 结束迭代返回false
 */
bool kl_trace_mutex_iter_locks(void** iter_tmp, kl_thread_t* owner,
